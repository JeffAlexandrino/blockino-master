...
Blockly.Toolbox.prototype.populate_ = function(newTree) {
  ...
  rootOut.removeChildren();  //cssItemLabel Delete any existing content.
  rootOut.blocks = [];
  function syncTrees(treeIn, treeOut) {
    for (var i = 0, childIn; childIn = treeIn.childNodes[i]; i++) {
      ...
      switch (childIn.tagName.toUpperCase()) {
        case 'CATEGORY':
          var childOut = rootOut.createNode(Blockly.Msg[childIn.getAttribute('name')]);
          //
          Blockino.toolbox_names.push(childIn.getAttribute('name'));
          childOut.blocks = [];
 ...
 * @param {Object} config The configureration for the tree. See
